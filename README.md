...
Churn Rate: $\frac{\text{Churned Customers}}{\text{Total Customers}}$  Retention Rate: $1 - \text{Churn Rate}$  Average Revenue Per User (ARPU): $\frac{\sum(\text{Monthly Charges})}{\text{Active Customers}}$  Average Customer Tenure: $\text{AVG}(\text{DATEDIFF}(\text{cancellation\_date OR NOW()}, \text{subscription\_start\_date}))$  Revenue at Risk: $\sum(\text{Monthly Charges}) \text{ WHERE } \text{churn\_score} > 70$  Escalation Rate: $\frac{\sum(\text{Escalations})}{\text{COUNT}(\text{Complaints})} \times 100$  
